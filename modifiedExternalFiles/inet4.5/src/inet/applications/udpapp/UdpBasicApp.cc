...
void UdpBasicApp::sendPacket()
{
    ...
    payload->setChunkLength(B(messageLength));
...
void UdpBasicApp::processSend()
{
    sendPacket();
    sendInterval = par("sendInterval");
    if (stopTime < CLOCKTIME_ZERO || getClockTime() + sendInterval < stopTime) {
        selfMsg->setKind(SEND);
        scheduleClockEventAfter(sendInterval, selfMsg);
    }
    else {
        selfMsg->setKind(STOP);
        scheduleClockEventAt(stopTime, selfMsg);
...
void UdpBasicApp::handleMessageWhenUp(cMessage *msg)
{
    ...
    // Added by Lars Breum Hansen M.sC, LTH
    } else if(msg->arrivedOn("controlIn")) {
        //std::cout << "Message received on controlIn" << endl;

        std::string cmd = msg->getName();

        if(cmd == "updateParams") {
        //std::cout << "UdpBasicApp has received an updateParamsMessage" << endl;

            try {
                std::cout << "sendInterval in udpBasicApp before: " << sendInterval << endl;

                messageLength = par("messageLength");
                sendInterval = par("sendInterval");

                std::cout << "updated udpBasicApp parameters" << endl;
                std::cout << "sendInterval in udpBasicApp after: " << sendInterval << endl;

            } catch (cException e) {
                std::cout << "error updating parameters on udpBasicApp" << endl;
            }
